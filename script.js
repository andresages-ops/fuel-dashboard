...
// 2. SWITCH CON COLORES DINÁMICOS
...
function exportarCSV() {
    ...
    let csv = "Fecha,Litros,Costo,Kilometraje,Gasolinera,Tipo,Notas\n";
    historial.forEach(r => {
        csv += `${r.fecha},${r.litros},${r.costo},${r.km},"${r.gasolinera}",${r.tipo},"${r.notas}"\n`;
    });
    ...
    a.setAttribute('download', `Gas_Respaldo_${new Date().toLocaleDateString()}.csv`);
...
saveBtn.onclick = () => {
    ...
    if(!l || !p || !km || !dateInput.value) return alert("Llena los datos obligatorios");
...
function cargarApp() {
    ...
    h.slice().reverse().forEach((reg, i) => {
        ...
        div.innerHTML = `<div class="icon-box"><i class="fas fa-gas-pump"></i></div><div class="details"><h4>${reg.fecha}</h4><p>${reg.tipo}</p></div><div class="cost"><h4>$${reg.costo.toFixed(0)}</h4></div>`;
        list.appendChild(div);
    });
}

// 6. REPORTES Y BORRADO
...
document.getElementById('delete-btn').onclick = () => { if(confirm("¿Borrar?")) { let h = JSON.parse(localStorage.getItem('fuelLogs')); h.splice(currentDetailIndex,1); localStorage.setItem('fuelLogs',JSON.stringify(h)); detailScreen.classList.add('hidden'); window.history.back(); cargarApp(); } };
addBtn.onclick = () => { dateInput.value = new Date().toISOString().split('T')[0]; addScreen.classList.remove('hidden'); window.history.pushState({m:1},""); };
